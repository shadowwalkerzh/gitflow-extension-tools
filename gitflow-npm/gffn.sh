...
merge() {
  local ORIGIN=$1
  local DESTINATION=$2
  git checkout $DESTINATION && git merge --no-ff --no-edit --no-commit -m "Automatic merge" $ORIGIN
}
...
finish_release() {
  ...
  local CURRENT_BRANCH=$(get_current_branch) && validate_branch $CURRENT_BRANCH && fast_commit && merge $CURRENT_BRANCH $MASTER_BRANCH && fast_commit
